...
J = [J_L(1,1)+m*(1.1)^2+J_R(1,1)+m*(1.1)^2 0 0  
     0 J_L(2,2)+m*(1.1)^2+J_R(2,2)+m*(1.1)^2 0
     0 0 J_L(3,3)+J_R(3,3)];

beta = -0.5*Cd*ro_mars*l_L*l_L;

%Posições dos retrorockets relativamente aos 3 primeiros modos ou pontos...
% de eq
p1 = [-1.35;-1.5;0];
p2 = [1.35;-1.5;0];
p3 = [1.35;1.5;0];
...
T_1 = 0.25*(mt*g_mars+beta*vz1^2)/cos(deg2rad(30)); %Existência de drag durante a descida do conjunto

a_3 = [0, -vx1*sin(theta1)+vz1*cos(theta1), -vx1*sin(psi1)-vy1*cos(psi1)
...
A_1 = [ zeros(3), Euler2R(lbd1), a_3, zeros(3)
      zeros(3), [zeros(2,3);0 0 2*beta*vz1/mt], skew(g_mars*z_I), skew(v1)
...
T2 = 0.25*(mt*g_mars+beta*vz2^2)/cos(deg2rad(30)); %Existência de drag durante a descida do conjunto
...
A_2 = [ zeros(3), Euler2R(lbd2), a_2, zeros(3)
     zeros(3), [zeros(2,3);0 0 2*beta*vz2/mt], skew(g_mars*z_I), skew(v2)
...
T3 = (0.25*mt*g_mars)/cos(deg2rad(30));
...
A_3 = [ zeros(3), Euler2R(lbd3), a_3, zeros(3)
     zeros(3), [zeros(2,3);0 0 2*beta*vz3/mt], skew(g_mars*z_I), skew(v3)
...
B_3 = [zeros(3,4)
   ...
   J^-1*n1, J^-1*n2, J^-1*n3, J^-1*n4];
%%
%Ponto de Equilibrio 4

J = [(1/12)*m*(l_L^2+h_L^2) 0 0  %Momento de Inércia do Lander
      0 (1/12)*m*(w_L^2+h_L^2) 0   
      0 0 (1/12)*m*(l_L^2+w_L^2)];
...
T4 = (m*g_mars*0.25)/(2*cos(deg2rad(29))+2*cos(deg2rad(31)));
px4 = 0;
py4 = 0;
pz4 = -21;
p4 = [px4;py4;pz4];
...
R = 0.1*eye(4);
%1
Q1 = blkdiag(1,1,1000,1,1,10,0.01*eye(3),0.001*eye(3));
%2
Q2 = blkdiag(1,1,1000,1,1,10000,0.01*eye(3),0.001*eye(3));
%3
Q3 = blkdiag(1,1,10,1,1,100,0.01*eye(3),0.001*eye(3));
%4
Q4 = blkdiag(1,10,100,1,100,10,0.01*eye(3),0.001*eye(3));
...
%%
%Estabilidade dos controladores
nmeas = 12;
ncont = 4;
...
%1
r1 = [0;0;-1050;0;0;0;0;0;0;0;0;0];
x1 = [0;0;-2100;0;0;89;0;0;0;0;0;0];
%2
r2 = [0;0;-21;0;0;0;0;0;0;0;0;0];
x2 = [0;0;-1050;0;0;44.5;0;0;0;0;0;0];
%3
r3 = [0;0;-21;0;0;0;0;0;0;0;0;0];
x3 = [0;0;-21;0;0;0;0;0;0;0;0;0];
%4
r4 = [0;1000;-21;0;0;0;0;0;0;0;0;0];
x4 = [0;0;-21;0;4;0;deg2rad(1);0;0;0;0;0];
...
for k = 1:NSim
    if x(3,k) >= -2100 && x(3,k) < -1050 && k<=aux
        i_ctr = 1;
        x(:,1) = x1;
        r(:,k) = [0;0;-1050;0;0;0;0;0;0;0;0;0]*(t(k)>=0);    

    elseif x(3,k) >= -1050 && x(3,k) < -21 && k<=aux
        i_ctr = 2;
        x(:,1)=x2;
    ...
    elseif k>=aux
        i_ctr = 4;
        x(:,1)=x4;
        r(:,k) = [0;100;-21;0;0;0;0;0;0;0;0;0]*(t(k)>=0);     
...
figure(4);
plot(t,r(7,:),t,r(8,:),t,y(7,:),t,y(8,:),t,y(9,:));
ylabel('Ângulos de Euler [rad]');
xlabel('Tempo [s]');
legend('r_phi','r_theta','phi','theta','psi');
grid on;

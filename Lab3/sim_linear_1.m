...
lbd = [phi;theta;psi];
x0 = [p; v; lbd; omega];
T = 0.25*(mt*g_mars+beta*vz^2)/cos(deg2rad(30)); %Existência de drag durante a descida do conjunto
...
figure(1);
plot(t,y_L(:,1),'k',t,y_L(:,2),'r',t,y_L(:,3),'g',t,y_L(:,6),'b');
...
%LQR
Q = blkdiag(1,1,100,1,1,10,0.01*eye(3),0.001*eye(3));
R = 0.1*eye(4);
Klqr = lqr(A,B,Q,R);
...
t = 0:Dt:60;
NSim = length(t);
r = [0;0;-1050;0;0;0;0;0;0;0;0;0]*(t>=0);
nx = 12;
nu = 4;
x = zeros(nx,NSim);
xu = zeros(nx,NSim);
